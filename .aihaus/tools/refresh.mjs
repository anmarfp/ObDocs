...
const hostAdapterPaths = new Set([
  ...
  // retired adapter paths from pre-rename installs stay excluded from evidence
...
async function execute(options) {
  ...
  if (options.status) {
    ...
    result.status = {
      ...
      memoryGaps: memoryGapsFor(packet),
      staleClaims: await staleClaimsFor(repository, packet),
    };
    return result;
  }
...
function usage() {
  return [
    "Usage:",
    "  node .aihaus/tools/refresh.mjs --repo <git-root> [--json]",
    "  node .aihaus/tools/refresh.mjs --repo <git-root> --dry-run [--json]",
    "  node .aihaus/tools/refresh.mjs --repo <git-root> --status [--json]",
    "",
    "Discovers local repository evidence without network access and writes only",
    packetRelative + ". Read .aihaus/REFRESH.md for the agent synthesis phase.",
  ].join("\n");
}
